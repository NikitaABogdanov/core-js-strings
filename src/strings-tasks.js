...
function removeFirstOccurrences(str, value) {
  const substrStartIndex = str.indexOf(value);
  const substrLastIndex = value.length + substrStartIndex;
  return substrStartIndex >= 0
    ? str.slice(0, substrStartIndex) + str.slice(substrLastIndex)
    : str;
}
...
function removeLastOccurrences(str, value) {
  const substrStartIndex = str.lastIndexOf(value);
  const substrLastIndex = value.length + substrStartIndex;
  return substrStartIndex >= 0
    ? str.slice(0, substrStartIndex) + str.slice(substrLastIndex)
    : str;
...
function sumOfCodes(str = '') {
  let sumOfCodePoints = 0;
  if (typeof str !== 'string') {
    return 0;
  }
  for (let i = 0; i < str.length; i += 1) {
    sumOfCodePoints += str.charCodeAt(i);
  }
  return sumOfCodePoints;
}
...
function formatTime(minutes, seconds) {
  let preparedMinutes = String(minutes);
  let preparedSeconds = String(seconds);
  if (minutes < 10) {
    preparedMinutes = preparedMinutes.padStart(2, '0');
  }
  if (seconds < 10) {
    preparedSeconds = preparedSeconds.padStart(2, '0');
  }
  return `${preparedMinutes}:${preparedSeconds}`;
}
...
function isPalindrome(str) {
  let result = '';
  const preparedStr = str.toLowerCase().replaceAll(/[!?,.:' ]/g, '');
  for (let i = 0; i < preparedStr.length; i += 1) {
    result = preparedStr[i] + result;
  }
  return result === preparedStr;
}
...
function encodeToRot13(str) {
  ...
  arr.forEach((char) => {
    if (
      (char.codePointAt() > 109 && char.codePointAt() < 123) ||
      (char.codePointAt() > 77 && char.codePointAt() < 91)
    ) {
      result.push(
        char.replace(char, String.fromCharCode(char.codePointAt() - 13))
      );
    } else if (
      (char.codePointAt() > 96 && char.codePointAt() <= 109) ||
      (char.codePointAt() > 64 && char.codePointAt() <= 77)
    ) {
      result.push(
        char.replace(char, String.fromCharCode(char.codePointAt() + 13))
      );
